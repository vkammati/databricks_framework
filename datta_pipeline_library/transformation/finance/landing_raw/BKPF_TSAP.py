...
#BKPF_TSAP Transform Load Landing to Raw
...
def load_BKPF_TSAP(spn, base_config):
        
    #:param spn: Azure SPN used to call the Databricks REST API
    #:param base_config: BaseConfig object
        
    logging.info("Started data load for BKPF_TSAP from Landing to Raw")
...
StructField("SLTTESTF1", StringType(), True),
StructField("SLTTEST123", StringType(), True),

    ])
    )
    dataset_name = "BKPF_TSAP"
    ...
    logging.info("Completed data load for BKPF_TSAP from Landing to Raw")

...
BSEG_STN_key=['MANDT', 'BUKRS','BELNR','GJAHR','BUZEI']
delta_load_source = spark.conf.get("pipeline.delta_load_source")
BSEG_STN_sequence_by = get_sequence_column(delta_load_source)
BSEG_STN_scd_type = 1
...
def BSEG_STN(df):
    ...
    print("Inside transformation/raw_euh/BSEG_STN.py",df)
    return (df)